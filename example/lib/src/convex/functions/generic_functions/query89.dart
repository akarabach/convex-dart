...
BTreeMapStringValue serialize(Query89Args args) {
  return hashmapToBtreemap(
    hashmap: {
      'i': encodeValue({
        for (final on214006 in args.i.entries)
          on214006.key: encodeValue(encodeValue(on214006.value)),
      }),
    },
  );
}
...
Query89Response deserialize(Value map) {
  return (decodeValue(map) as IMap<String, dynamic>).then(
    (on367780) => (
      i: (on367780['i'] as IMap<String, dynamic>).map(
        (on638242, on908800) => MapEntry(
          on638242,
          on908800 == null ? null : $1_0.validate(on908800),
        ),
      ),
    ),
  );
