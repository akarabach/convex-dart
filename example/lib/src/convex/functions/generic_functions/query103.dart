...
BTreeMapStringValue serialize(Query103Args args) {
  return hashmapToBtreemap(
    hashmap: {
      'i': encodeValue(
        args.i.split(
          (on219079) => encodeValue(on219079),
          (on426481) => encodeValue(on426481),
        ),
      ),
    },
  );
...
Query103Response deserialize(Value map) {
  return (decodeValue(map) as IMap<String, dynamic>).then(
    (on837101) => Query103Response(
      i: Union2<int, double>(() {
        try {
          return (on837101['i'] as int);
        } catch (e) {}

        try {
          return (on837101['i'] as double);
        } catch (e) {}

        throw Exception(
          (on837101['i'].toString() ?? "null") +
