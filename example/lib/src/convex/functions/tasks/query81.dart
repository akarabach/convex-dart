...
BTreeMapStringValue serialize(Query81Args args) {
  return hashmapToBtreemap(
    hashmap: {
      'i': encodeValue({
        for (final on711038 in args.i.entries)
          on711038.key: encodeValue(encodeValue(on711038.value)),
      }),
    },
  );
}
...
Query81Response deserialize(DartValue map) {
  return (decodeValue(map) as IMap<String, dynamic>).then(
    (on615797) => (
      i: (on615797['i'] as IMap<String, dynamic>).map(
        (on501423, on533826) => MapEntry(on501423, (on533826 as String?)),
      ),
    ),
  );
}
