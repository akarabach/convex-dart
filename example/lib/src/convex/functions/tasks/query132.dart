  ...
  return hashmapToBtreemap(
    hashmap: {
      'i': encodeValue({
        'a': encodeValue(
          args.i.a
              .map((on86088) => encodeValue({'b': encodeValue(on86088.b)}))
              .toIList(),
        ),
      }),
    },
...
Query132Response deserialize(DartValue map) {
  return (decodeValue(map) as IMap<String, dynamic>).then(
    (on995931) => (
      i: (on995931['i'] as IMap<String, dynamic>).then(
        (on668928) => (
          a: (on668928['a'] as IList<dynamic>)
              .map(
                (on671437) => (on671437 as IMap<String, dynamic>).then(
                  (on126749) => (b: (on126749['b'] as String)),
                ),
              )
              .toIList(),
        ),
