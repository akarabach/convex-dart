...
Query129Response deserialize(DartValue map) {
  return (decodeValue(map) as IMap<String, dynamic>).then(
    (on485539) => (
      i: (on485539['i'] as IMap<String, dynamic>).then(
        (on323885) => (
          error: on323885.containsKey('error')
              ? Defined(
                  (on323885['error'] as IMap<String, dynamic>).then(
                    (on45584) => (
                      handler: (on45584['handler'] as String),
                      retry: (on45584['retry'] as IMap<String, dynamic>).then(
                        (on246156) => (
                          count: (on246156['count'] as double),
                          delay: (on246156['delay'] as double),
                        ),
                      ),
                    ),
                  ),
