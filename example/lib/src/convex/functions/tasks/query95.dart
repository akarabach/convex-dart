...
BTreeMapStringValue serialize(Query95Args args) {
  return hashmapToBtreemap(
    hashmap: {
      'i': encodeValue(
        args.i.split(
          (on698239) => encodeValue(on698239),
          (on926481) => encodeValue(on926481),
          (on77766) => encodeValue(on77766),
        ),
      ),
    },
  );
  ...
  return (decodeValue(map) as IMap<String, dynamic>).then(
    (on28383) => (
      i: Union3<String, double, bool>(() {
        try {
          return (on28383['i'] as String);
        } catch (e) {}

        try {
          return (on28383['i'] as double);
        } catch (e) {}

        try {
          return (on28383['i'] as bool);
        } catch (e) {}

        throw Exception(
          (on28383['i'].toString() ?? "null") +
