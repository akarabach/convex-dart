...
BTreeMapStringValue serialize(Query167Args args) {
  return hashmapToBtreemap(
    hashmap: {
      'i': encodeValue({
        for (final on425171 in args.i.entries)
          on425171.key: encodeValue(encodeValue(on425171.value)),
      }),
    },
  );
}
...
Query167Response deserialize(DartValue map) {
  return (decodeValue(map) as IMap<String, dynamic>).then(
    (on396404) => (
      i: (on396404['i'] as IMap<String, dynamic>).map(
        (on690761, on928417) => MapEntry(on690761, PostsId(on928417 as String)),
      ),
    ),
  );
}
