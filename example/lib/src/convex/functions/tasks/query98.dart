...
BTreeMapStringValue serialize(Query98Args args) {
  return hashmapToBtreemap(
    hashmap: {
      'i': encodeValue(
        args.i.split(
          (on708745) => encodeValue(on708745),
          (on584295) => encodeValue(on584295),
        ),
      ),
    },
  );
  ...
  return (decodeValue(map) as IMap<String, dynamic>).then(
    (on415047) => (
      i: Union2<Uint8ListWithEquality, String>(() {
        try {
          return (on415047['i'] as Uint8ListWithEquality);
        } catch (e) {}

        try {
          return (on415047['i'] as String);
        } catch (e) {}

        throw Exception(
          (on415047['i'].toString() ?? "null") +
