...
BTreeMapStringValue serialize(Query197Args args) {
  return hashmapToBtreemap(
    hashmap: {
      'i': encodeValue({
        for (final on980201 in args.i.entries)
          on980201.key: encodeValue(encodeValue(on980201.value)),
      }),
    },
  );
}
...
Query197Response deserialize(DartValue map) {
  return (decodeValue(map) as IMap<String, dynamic>).then(
    (on486028) => (
      i: (on486028['i'] as IMap<String, dynamic>).map(
        (on256738, on147252) => MapEntry(on256738, (on147252 as double)),
      ),
    ),
  );
}
