...
BTreeMapStringValue serialize(Query102Args args) {
  return hashmapToBtreemap(
    hashmap: {
      'i': encodeValue(
        args.i.split(
          (on73872) => encodeValue(on73872),
          (on669919) => encodeValue(on669919),
          (on684650) => encodeValue(on684650),
        ),
      ),
    },
  );
  ...
  return (decodeValue(map) as IMap<String, dynamic>).then(
    (on366208) => (
      i: Union3<$admin, $user, $guest>(() {
        ...
        if (map.containsKey(on366208['i'])) {
          return map[on366208['i']];
        }
        throw Exception(
          (on366208['i'].toString() ?? "null") +
