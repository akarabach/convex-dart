...
BTreeMapStringValue serialize(Query71Args args) {
  return hashmapToBtreemap(
    hashmap: {
      'i': encodeValue({
        for (final on66213 in args.i.entries)
          on66213.key: encodeValue(encodeValue(on66213.value)),
      }),
    },
  );
}
...
Query71Response deserialize(DartValue map) {
  return (decodeValue(map) as IMap<String, dynamic>).then(
    (on104767) => (
      i: (on104767['i'] as IMap<String, dynamic>).map(
        (on449002, on895018) => MapEntry(on449002, (on895018 as bool)),
      ),
    ),
  );
}
