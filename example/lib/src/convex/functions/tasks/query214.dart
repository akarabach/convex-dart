  ...
  return hashmapToBtreemap(
    hashmap: {
      'i': encodeValue(
        args.i
            .map(
              (on984556) => encodeValue({
                'id': encodeValue(on984556.id),
                'score': encodeValue(on984556.score),
              }),
            )
            .toIList(),
      ),
...
Query214Response deserialize(DartValue map) {
  return (decodeValue(map) as IMap<String, dynamic>).then(
    (on30482) => (
      i: (on30482['i'] as IList<dynamic>)
          .map(
            (on68306) => (on68306 as IMap<String, dynamic>).then(
              (on811798) => (
                id: (on811798['id'] as String),
                score: (on811798['score'] as double),
              ),
            ),
          )
          .toIList(),
