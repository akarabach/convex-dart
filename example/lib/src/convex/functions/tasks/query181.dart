...
Query181Response deserialize(DartValue map) {
  return (decodeValue(map) as IMap<String, dynamic>).then(
    (on283594) => (
      i: (on283594['i'] as IMap<String, dynamic>).then(
        (on246749) => (
          deeply: (on246749['deeply'] as IMap<String, dynamic>).then(
            (on175222) => (
              nested: (on175222['nested'] as IMap<String, dynamic>).then(
                (on220776) => (value: (on220776['value'] as String)),
              ),
            ),
          ),
        ),
